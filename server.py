...
@server.tool(title="Read Topic Frequencies")
def read_topic_frequencies(
    robolog_path: str,
    topics: list[str] | None = None,
    start_seconds: float | None = None,
    end_seconds: float | None = None,
) -> list[dict[str, Any]]:
    """Return topic message frequencies for the specified topics and time range.

    Each dictionary in the list represents a frequency measurement with the following structure:
    - robolog_id: Unique identifier for the robolog, generated by reading the robolog content.
    - timestamp_seconds: Timestamp when the frequency was measured in seconds. Please note that
        this is not always the Unix epoch seconds. For example, it can be the time when the robot
        was recording messages.

    The rest of the keys in the dictionary are the topic names, and their values will be the
    corresponding message frequency (Hz) at that timestamp. Frequency values are floating-point
    numbers representing messages per second.

    Args:
        robolog_path (str): Path to the robolog.
        topics (list[str] | None, optional): Topics to read frequencies from. If None, all topics
            are included.
        start_seconds (float | None, optional): When to start reading frequencies.
        end_seconds (float | None, optional): When to stop reading frequencies.

    Returns:
        list[dict[str, Any]]: A list of dictionaries, each representing topic frequencies at a
            specific timestamp.

    """
    reader = factory.make_topic_frequency_reader(robolog_path)
    dataset = reader.read(topics=topics, start_seconds=start_seconds, end_seconds=end_seconds)
    return dataset.to_table().to_pylist()
